...
void ObjUtils::addElement(std::string type, int tag, std::vector<int> nodeTags, std::vector<std::string> line)
{
	...
	else if (type.compare("ElasticTimoshenkoBeam") == 0)
	{
		pEle = new CSSElasticTimoshenkoBeam(tag, nodeTags);
	...
	else if (type.compare("ShellMITC4") == 0)
	{
		pEle = new CSSShellElement(tag, nodeTags);
	}
